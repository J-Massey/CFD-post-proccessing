...
# t, fx, fy = io.unpack_flex_forces(length_scale, file)
print(io.unpack_profiles('./39e3_32/profiles/',256,3,theta=p.pi/32))

# print(np.sort(os.listdir('./39e3_32/profiles/')))
# t=t[t/length_scale>=200]
# fy = fy[-t.shape[0]:]
# plotter.plotCL(fy, t/length_scale, 'CL-t.pdf', St=cf.find_St(t,fy,length_scale,U), CL_rms=cf.rms(fy), CD_rms=cf.rms(fx), n_periods=cf.find_num_periods(fy))
