...
def main():
    ...
    for idx in range(len(xs[0])):
        for case,(x,y) in enumerate(zip(xs,ys)):
            ...
            #             y_label=r"$ \ln[\mathcal{L}(\mu_k|x_k)] $", x_label=r"$ t/D $", colour=colours[case],
...
#             title=f"$ {round(angles[idx], 2)}^r $ from the front", alpha=0.4, color=colours[case], colours=colours[:len(xs)], label=labels)
#     plt.close()
