...
def compare_data(model, poly_n: int, device="cuda", angles=32, fn='model.pdf'):
    ...
    poly = poly.fit_transform(p_data[:, 0:-1])
    p_data = np.concatenate((poly, p_data[:, -1][..., np.newaxis]), axis=1)
...
def main(poly_n, eps):
    ...
    compare_model(model, poly_n, fn='figures/model_gt_n_' + str(poly_n) + '.pdf')
    try:
        compare_data(model, poly_n, fn='figures/model_data_n_' + str(poly_n) + '.pdf')
    finally:
...
if __name__ == "__main__":
    # main(5, 1e3)
    for n, e in zip([1, 2, 5], [1e4, 1e4, 1e4]):
        main(n, e)
