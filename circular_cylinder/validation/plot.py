...
def plot_pressure():
    ...
    p_norberg = np.loadtxt('norberg_p_theta.csv', delimiter=',')
    p = np.mean(p, axis=1)
    ...
    ax.scatter(np.linspace(0, np.pi/2 - (np.pi/2/len(p)), len(p)), p * 2,
               label=r'BDIM', color='red', marker='X')
    ax.scatter(np.pi/2 * p_norberg[:, 0]/180, p_norberg[:, 1],
               label=r'Norberg (1987)', color='blue', marker='d')
