...
def res_test(data_root, cs, case, **kwargs):
    ...
    if mean:
        ax.set_ylabel(r'$C_{D_p}$')
    else:
        ax.set_ylabel(r'$RMS(C_{D_p}^{\prime})$')
    ...
    for idx, res in enumerate(cs):
        force_file = str(res) + '/3D/fort.9'
        names = ['t', 'dt', 'px', 'py', 'pz']
        interest = 'p'
        fos = (io.unpack_flex_forces(os.path.join(data_root, force_file), names))
        forces_dic = dict(zip(names, fos))
        t = forces_dic['t']/res
        print(min(t))
        u, uy = (forces_dic[interest + 'x'], forces_dic[interest + 'y'])
...
if __name__ == '__main__':
    c = [64, 96, 128, 192, 256, 512]
    # flow_res_test()
    # plot_coeffs(drag=True)
    data_root = '/home/masseyjmo/Workspace/Lotus/projects/waving_plate/'
    res_test(data_root, c, 'smooth', mean=True)
    res_test(data_root, c, 'smooth', rms=True)
    # c = [128, 192, 256]
    TS(data_root, 'smooth')
