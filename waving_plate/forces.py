...
cases = ['full_bumps', 'smooth', 'half_bumps', 'riblet']
labels = [r'$ 70\% $', r'Smooth', r'$ 36\% $', r'Riblet']


def read_rot(data_root, c, interest='p'):
    force_file = 'fort.9'
    names = ['t', 'dt', 'px', 'py', 'pz', 'cpx', 'cpy', 'cpz', 'cfx', 'cfy', 'cfz']
    fos = (io.unpack_flex_forces(os.path.join(data_root, str(c), force_file), names))
    forces_dic = dict(zip(names, fos))
    t = (0.3 / (0.2 * c)) * (forces_dic['t'])
    ux = np.squeeze(forces_dic[interest + 'x'])
    uy = np.squeeze(forces_dic[interest + 'y'])
    uz = np.squeeze(forces_dic[interest + 'z'])
...
def cf(cases=cases):
    ...
    for idx2, case in enumerate(cases):
        ...
        t, u, _, _ = read_rot(data_root, c, interest='cf')

        t, u = t[t < snip], u[t < snip]
        t, u = t[t > init], u[t > init]

        ax1.plot(t, u, label=labels[idx2], color=colors[idx2])
    ...
    ax1.legend()
    fig1.savefig(os.path.join(data_root, '../comparisons/cf_comparison.png'),
                 bbox_inches='tight', dpi=600, transparent=True)
    postproc.visualise.plotter.plotLogLogTimeSpectra_list(
        os.path.join(data_root, '../comparisons/log_spectra_cf_comparison.png'),
        uks_labelled, fs,
        ylabel=r'$PS$ ' + label,
        colors=colors)
    plt.close()
...
def cp(cases=cases):
    ...
    for idx2, case in enumerate(cases):
        ...
        t, u, _, _ = read_rot(data_root, c, interest='cp')
    ...
    ax1.legend()
    fig1.savefig(os.path.join(data_root, '../comparisons/cpower_comparison.png'),
                 bbox_inches='tight', dpi=600, transparent=True)
    postproc.visualise.plotter.plotLogLogTimeSpectra_list(
        os.path.join(data_root, '../comparisons/log_spectra_cpower_comparison.png'),
        uks_labelled, fs,
        ylabel=r'$PS$ ' + label,
        colors=colors)
    plt.close()
...
def cd(cases=cases):
    ...
    for idx2, case in enumerate(cases):
        ...
        t, u, _, _ = read_rot(data_root, c, interest='p')
    ...
    ax1.legend()
    fig1.savefig(os.path.join(data_root, '../comparisons/cd_comparison.png'),
                 bbox_inches='tight', dpi=600, transparent=True)
    postproc.visualise.plotter.plotLogLogTimeSpectra_list(
        os.path.join(data_root, '../comparisons/log_spectra_cd_comparison.png'),
        uks_labelled, fs,
        ylabel=r'$PS$ ' + label,
        colors=colors)
    plt.close()
...
def efficiency(cases=cases):
    ...
    for idx2, case in enumerate(cases):
        ...
        td, ud, _, _ = read_rot(data_root, c, interest='p')
        tp, up, _, _ = read_rot(data_root, c, interest='cp')

        u = np.mean(ud[td > init])/np.mean(up[tp > init])

        ax1.scatter(idx2, u, label=labels[idx2], color=colors[idx2], marker='P', s=100)

    ax1.set_xticks(range(len(cases)))
    ax1.set_xticklabels(labels)
    fig1.savefig(os.path.join(data_root, '../comparisons/efficiency_comparison.png'),
                 bbox_inches='tight', dpi=600, transparent=True)
